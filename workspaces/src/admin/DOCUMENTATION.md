...
- **Git Backup** (`src/admin/git/`): Configuration, cloning, authentication
  ...
  - `sync.py`: `sync_once()` commits and pushes the changes in one
    ...
    to commit and raising `SyncError` on failure - extended with the same
    invalid/expired-token hint as `clone.py` when a push is rejected for
    credentials. Commit messages carry a UTC timestamp. The commit
...
| `tests/test_git_sync.py` | `admin/git/sync.py` | Changes detected, no changes, failed push and commit, commit identity, timestamped message, credential handling, clear error on invalid/expired token with the token never in the message |
